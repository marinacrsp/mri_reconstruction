...
class KCoordDataset(Dataset):
    ...
    def __init__(
        ...
        for vol_id, file in enumerate(files):
            ...
            kspace_coords[:, :2] = kspace_ids[:, :2] # Unnormalized x, y
            kspace_coords[:, 2] = (2 * kspace_ids[:, 2]) / (n_slices - 1) - 1 # Normalized zcoordinate
            kspace_coords[:, 3] = (2 * kspace_ids[:, 3])/ (n_coils - 1) - 1 # NOTE: Normalized coilID
