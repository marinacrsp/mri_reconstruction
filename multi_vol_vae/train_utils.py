...
class Trainer:
    def __init__(
        self, dataloader, model, vae, loss_fn, optimizer, scheduler, config
        ) -> None:
        self.device = torch.device(config["device"])
        self.n_epochs = config["n_epochs"]
        self.scale = config['loss']['kl_scale']
        self.dataloader = dataloader
        self.model, self.vae = model.to(self.device), vae.to(self.device)
        self.grid = self.dataloader.dataset.coords.to(self.device)
        ...
        self.kspace_volumes = self.dataloader.dataset.dict_volumes.to(self.device)
        ...
        for vol_id in self.dataloader.dataset.metadata.keys():
            ...
            with h5py.File(file, "r") as hf:
                self.ground_truth.append( 
                    hf["reconstruction_rss"][()][: config["dataset"]["n_slices"]].squeeze(0) # squeeze bc there is only 1 slice
                )
                self.kspace_gt.append(
                    to_tensor(preprocess_kspace(hf["kspace"][()][: config["dataset"]["n_slices"]].squeeze(0)
                )
                    )
    ...
    def train(self):
        ...
        for epoch_idx in range(self.n_epochs):
            empirical_risk, empirical_recon, empirical_kld = self._train_one_epoch(epoch_idx)
    ...
    def _train_one_epoch(self, epoch_idx):
        ...
        # for batch_idx, (normalized_inputs, inputs, targets) in enumerate(self.dataloader):
        inputs, targets = inputs.to(self.device), targets.to(self.device)
            
        # Get the index for the coil latent embedding
        coords = inputs[:, 1:] # x, y, coilID 
        ...
        # VAE, sample latent
        posterior = self.vae.encode(self.kspace_volumes)
        pe = self.vae.decode(posterior.sample())
        
        # Interpolate the grid of embeddings, filter the corresponding coordinates in batch, at multiresolution setup
        level_embd = []
        for i in range(len(pe)):
            interpolated_grid_embds = self.singleplane_positional_encoding(pe[i], self.grid) # Function that performs bilinear interp
            # input: is the sampled pe embedding [bsz, ch_dim, 128, 128]; grid : [bsz, 320, 320, 2]
            # output: is the interpolated grid of embeddings : [bsz, ch_dim, 320, 320] # chdim: 64
            level_embd.append(interpolated_grid_embds[vol_ids,:,coords[:,0], coords[:,1]]) # find the batch embeddings at multiresolution
        outputs = self.model(coords, level_embd)
        
        ## Reconstruction loss
        recon_loss = self.loss_fn(outputs, targets)
        
        # kl_loss
        kld = posterior.kl(mean=False)
        kld_loss = self.scale*torch.mean(kld)
            
        # Combined loss
        batch_loss = recon_loss + kld_loss
        ...
        return batch_loss, avg_recon, avg_kld
    ...
    def predict(self, vol_id, shape, left_idx, right_idx, center_vals):
        ...
        for point_ids in dataloader:
            ...
            vol_ids = torch.tensor([vol_id] * len(point_ids), dtype=torch.long, device=self.device)
            ...
            for i in range(len(pe)):
                interpolated_grid_embds = self.singleplane_positional_encoding(pe[i], self.grid) # Function that performs bilinear interp
                # [volid, coilid, kx, ky]
                level_embd.append(interpolated_grid_embds[vol_ids,:,point_ids[:,0], point_ids[:,1]]) # find the batch embeddings at multiresolution
            ...
            coords[:, 2] = (2 * point_ids[:, 2]) / (n_coils - 1) - 1

            # Need to add `:len(coords)` because the last batch has a different size (than 60_000).
            outputs = self.model(coords, level_embd)
            # "Fill in" the unsampled region.            
            volume_kspace[
                point_ids[:, 2], point_ids[:, 1], point_ids[:, 0]
            ] = outputs


        # Multiply by the normalization constant.
        volume_kspace = tensor_to_complex_np(volume_kspace.detach().cpu() * self.dataloader.dataset.metadata[vol_id]["norm_cste"])
    ...
    def _log_performance(self, epoch_idx):
        for vol_id in self.dataloader.dataset.metadata.keys():
            ...
            self._plot_2subplots(np.log(volume_kspace_rss/cste_mod + 1.e-45), 'kspace predicted',
                                ...
                                epoch_idx, 
                                f"prediction/vol_{vol_id}/kspace logarigthm", 'viridis')
    ...
    @torch.no_grad()
    def _plot_3subplots(
        self, data_1, title1, data_2, title2, data_3, title3, epoch_idx, tag, map
        ):
        fig = plt.figure(figsize=(20,30))
        plt.subplot(1,3,1)
        plt.imshow(data_1, cmap=map)
        plt.title(title1)
        plt.axis('off')
        
        plt.subplot(1,3,2)
        plt.imshow(data_2, cmap=map)
        plt.title(title2)
        plt.axis('off')
        
        plt.subplot(1,3,3)                
        plt.imshow(data_3, cmap=map)
        plt.title(title3)
        plt.axis('off')
    ...
    @torch.no_grad()
    def _plot_2subplots(
        self, data_1, title1, data_2, title2, epoch_idx, tag, map
        ):
    ...
    def _log_weight_info(self, epoch_idx):
        ...
        for module, case in zip(
            [self.model, self.vae], ["network", "vae"]
        ):
