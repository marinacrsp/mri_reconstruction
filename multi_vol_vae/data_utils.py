...
def rss(data: np.ndarray) -> np.ndarray:
    ...
    if len(data.shape) == 5: # nslices, ncoils, width, height, 2
        return np.sqrt((np.abs(data) ** 2).sum(1))
    
    else: # ncoils, width, height, 2
        if len(data.shape) == 4:
            _, w, h, _ = data.shape
            assert w == h == 320, 'Wrong dimension'
        elif len(data.shape) == 3:
            _, w, h = data.shape
            assert w == h == 320, 'Wrong dimension'
            
        return np.sqrt((np.abs(data) ** 2).sum(0))
...
def combine_phase(coil_imgs, coil_axis):
    """
    Compute a combine phase image from multiple coils.
    Combination is done as weighted sum (weights are magnitude of coil image)

        pha_comb = angle( sum_c |img_c|^2 * exp(1j * angle(img_c)) )

    Note:
        Phase offsets from receiver channels should have been accounted for,
        before using this funciton to combine phases...

    Parameters
    ----------
        coil_imgs: NumPy array. Shape (num_coils, Nx, Ny)
    """
    if isinstance(coil_imgs, np.ndarray):
        return np.angle(
            np.sum(
                np.abs(coil_imgs) ** 2 * np.exp(1j * np.angle(coil_imgs)),
                axis=coil_axis,
            )
        )
    else:
        raise ValueError(f"Cannot handle type {type(coil_imgs)}")
