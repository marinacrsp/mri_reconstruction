...
class ModulatedConv2d(nn.Module):
    """
    Scaled convolution weights dynamically and adaptively to a style vector (latent)
    """
...
class StyledConv(nn.Module):
    ...
    def forward(self, input, style, noise=None):
        out = self.conv(input, style) # Input and latent (style -> shape bsz, ch size?)
...
class LFF(nn.Module):
    def __init__(self, input_size, hidden_size, ):
        ...
        self.ffm = ConLinear(input_size, hidden_size, is_first=True)
...
class StyledResBlock(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, style_dim, blur_kernel=[1, 3, 3, 1], demodulate=True,
        ...
        self.conv1 = StyledConv(in_channel, out_channel, 1, style_dim,
        ...
        self.conv3 = StyledConv(out_channel, out_channel, 1, style_dim,
                                demodulate=demodulate, activation=activation,
                                upsample=upsample, downsample=downsample)
...
class StyledResBlock2(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, style_dim, blur_kernel=[1, 3, 3, 1], demodulate=True,
                 activation=None, upsample=False, downsample=False):
        super().__init__()
