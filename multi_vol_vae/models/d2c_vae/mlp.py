...
class MLP(nn.Module):
    ...
    def forward(self, coords, hdbf, si=1):
        ...
        scale_inj_pixel = torch.ones_like(coords) * si # to be concatenated w x, size: [bsz, ch, W, H]
        coords = coords.permute(0, 2, 3, 1).contiguous() # This is to have the size : [bsz, W, H, 1]
        ...
        ############################################################################################
        # hdbf[0] -> torch.Size([4, 64, 128, 128]), coords dim: [bsz, W, H, 1]
